...
2. **Run the scaffold script** — replaces all tokens and renames `addons.php → <slug>.php`
   ...
   |---|---|---|
   | `addons` | lowercase slug = text-domain = i18n domain | `restock` |
   | `Addons` | PSR-4 PHP namespace | `Restock` |
   | `ADDONS` (in `define()`) | uppercased namespace | `RESTOCK` |
   | `addons_` | option/meta prefix (slug, dashes→underscores) | `restock_` |
   | `Add-Ons – Product Options for WooCommerce` / `Let customers add paid or free options (text, checkbox, select) to products before adding to cart.` / `Let customers add paid or free options (text, checkbox, select) to products before adding to cart.` | name + descriptions | … |
...
- **Bootstrap** (`addons.php`): PHP/WC guards, HPOS + cart-blocks compat, `plugins_loaded`
  boot, `do_action('addons/booted')` (the hook a PRO companion extends).
